...
점수와 별개로 **자격 조건**이 둘 있습니다. 하나라도 못 채우면 점수가 39점(= 등급 없음)으로 묶입니다.

- **Stars 30개 이상** (`MIN_STARS`) — 아무도 별을 주지 않은 프로젝트는 등급 대상이 아닙니다. fork만 많은 강의·템플릿 repository를 거르는 역할도 합니다.
- **License 선언** — 없으면 정의상 오픈소스가 아닙니다. GitHub이 분류하지 못한 커스텀 License(`key: other`)는 "선언은 했다"로 보고 통과시킵니다.

Stars 30은 하한선이지 합격선이 아닙니다. 점수 계산까지 통과해야 하므로, 실제로 커뮤니티 등급이 붙는 것은 대략 Stars 60개 이상부터입니다.
