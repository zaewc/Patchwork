...
describe("GET /api/auth/login", () => {
  it("OAuth 설정이 없으면 안내와 함께 홈으로 되돌린다", async () => {
  ...
  ])("%s 하나만 빠져도 설정이 없는 것으로 본다", async (_label, missing) => {
    configure();
  ...
  it("state를 쿠키와 URL에 같은 값으로 심는다", async () => {
    configure();
  ...
  it("state 쿠키는 10분만 살아 있는 httpOnly 쿠키다", async () => {
    configure();

    const cookie = (await handleLogin(request())).cookies.get(STATE_COOKIE);

    expect(cookie).toMatchObject({ httpOnly: true, sameSite: "lax", path: "/", maxAge: 600 });
  });

  it("요청마다 다른 state를 만든다", async () => {
    configure();

    const first = (await handleLogin(request())).cookies.get(STATE_COOKIE)?.value;
    const second = (await handleLogin(request())).cookies.get(STATE_COOKIE)?.value;

    expect(first).not.toBe(second);
  });

  it("APP_URL이 있으면 그 주소로 되돌아오게 한다", async () => {
    configure();
