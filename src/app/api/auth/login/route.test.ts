...
describe("GET /api/auth/login", () => {
  ...
  it("state 쿠키는 10분만 살아 있는 httpOnly 쿠키다", async () => {
    ...
    const cookie = (await GET(request())).cookies.get(STATE_COOKIE);
  ...
  it("요청마다 다른 state를 만든다", async () => {
    ...
    const first = (await GET(request())).cookies.get(STATE_COOKIE)?.value;
    const second = (await GET(request())).cookies.get(STATE_COOKIE)?.value;
