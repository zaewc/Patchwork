...
import { SESSION_COOKIE, STATE_COOKIE, unseal } from "@/lib/session";

vi.mock("@/lib/github", () => ({ fetchViewerIdentity: vi.fn() }));
...
describe("GET /api/auth/callback · 토큰 교환", () => {
  ...
  it("토큰 요청이 실패하면 안내와 함께 홈으로 보낸다", async () => {
    ...
    expect(errorOf(await GET(request()))).toBe("token_exchange_failed");
  });

  it("GitHub이 준 오류 사유를 그대로 전달한다", async () => {
    fetchMock.mockResolvedValue(json({ error: "bad_verification_code" }));

    expect(errorOf(await GET(request()))).toBe("bad_verification_code");
  });
  ...
  it("사용자 정보를 못 가져오면 안내와 함께 홈으로 보낸다", async () => {
    ...
    expect(errorOf(await GET(request()))).toBe("identity_failed");
  });
});
...
describe("GET /api/auth/callback · 성공", () => {
  ...
  it("세션 쿠키는 30일짜리 httpOnly 쿠키다", async () => {
    const cookie = (await GET(request())).cookies.get(SESSION_COOKIE);
