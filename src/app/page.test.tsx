...
vi.mock("@/lib/session", () => ({ getSession: vi.fn(), isConfigured: vi.fn() }));
...
describe("HomePage", () => {
  ...
  it("서비스 소개를 보여준다", async () => {
    ...
    expect(
      screen.getByRole("heading", { level: 1, name: "오픈소스 기여를 한 장의 Patchwork로" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText("GitHub contribution과 진행 중인 pull request 상태를 추적합니다."),
    ).toBeInTheDocument();
  });
  ...
  describe("OAuth 앱이 설정되지 않은 경우", () => {
    ...
    it("콜백 URL과 환경변수 파일 이름을 알려준다", async () => {
      ...
      expect(screen.getByText("http://localhost:3000/api/auth/callback")).toBeInTheDocument();
