...
import { NOTABLE_IMPACT, PLAIN_IMPACT } from "@/lib/__fixtures__/dashboard";
import { fetchContributionItems, GitHubAuthError, type ContributionGroup } from "@/lib/github";
...
const renderPage = async (
  ...
  return render(await ExportPage(props(searchParams)));
};
...
describe("접근 제어", () => {
  it("세션이 없으면 홈으로 보낸다", async () => {
    ...
    await expect(ExportPage(props())).rejects.toBeInstanceOf(RedirectSignal);
    expect(redirect).toHaveBeenCalledExactlyOnceWith("/");
  });

  it("세션의 토큰과 조회 범위로 기여 목록을 가져온다", async () => {
    await renderPage([group()], { range: "5y" });
    expect(fetchContributionItems).toHaveBeenCalledExactlyOnceWith("gho_token", "5y");
  });

  it("토큰이 만료되면 다시 로그인하도록 보낸다", async () => {
    vi.mocked(fetchContributionItems).mockRejectedValue(new GitHubAuthError());
  ...
  it("로그인한 사용자를 헤더에 보여준다", async () => {
    await renderPage();
    expect(screen.getByRole("link", { name: "The Octocat" })).toBeInTheDocument();
  });
});
...
describe("머리글", () => {
  it("무엇을 만드는 화면인지 설명한다", async () => {
    ...
    expect(screen.getByRole("heading", { level: 1, name: "README 내보내기" })).toBeInTheDocument();
    expect(screen.getByText(/repository별로 묶어 Markdown으로 만듭니다/)).toBeInTheDocument();
  });
  ...
  it("탭은 export 경로를 가리킨다", async () => {
    ...
    expect(screen.getByRole("link", { name: "5년" })).toHaveAttribute("href", "/export?range=5y");
  });

  it("현재 조회 조건을 표시한다", async () => {
    await renderPage([group()], { range: "30d", scope: "all" });

    expect(screen.getByRole("link", { name: "30일" })).toHaveAttribute("aria-current", "page");
  ...
  it("repository 수와 항목 수를 센다", async () => {
    await renderPage([
      ...
      group({
        ...
        items: [
          { type: "PR", title: "a", url: "https://x/1", createdAt: "2026-01-01" },
          { type: "Issue", title: "b", url: "https://x/2", createdAt: "2026-01-02" },
        ],
      }),
    ]);
...
describe("주요 OSS 걸러내기", () => {
  it("기본은 주요 OSS만 담는다", async () => {
    ...
    expect(container.querySelector("pre")!.textContent).toContain("### next.js");
    expect(container.querySelector("pre")!.textContent).not.toContain("### toy");
...
describe("내보낼 것이 없을 때", () => {
  it("기간을 넓히거나 전체로 전환하라고 안내한다", async () => {
    ...
    expect(screen.queryByRole("button", { name: "Markdown 복사" })).not.toBeInTheDocument();
    expect(
      screen.getByText(
...
describe("조회 실패", () => {
  it("사유를 배너로 알리고 다시 시도하라고 안내한다", async () => {
    vi.mocked(fetchContributionItems).mockRejectedValue(new Error("GitHub API 오류 (HTTP 403)"));
    render(await ExportPage(props()));
  ...
  it("Error가 아닌 값으로 실패해도 화면은 그린다", async () => {
    vi.mocked(fetchContributionItems).mockRejectedValue("문자열 실패");
    render(await ExportPage(props()));

    expect(screen.getByText("기여 목록을 불러오지 못했습니다.")).toBeInTheDocument();
  });
});
