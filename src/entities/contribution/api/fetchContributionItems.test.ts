...
function mockSearch(
  reply: (
    isPullRequestSearch: boolean,
    page: number,
  ) => ReturnType<typeof searchItemsResponse>,
) {
...
describe("검색 한정자", () => {
  it("공개 저장소·기간·정렬을 쿼리에 담는다", async () => {
    ...
    expect(
      queriesOf().some((q) =>
        q.includes("is:issue is:closed reason:completed"),
      ),
    ).toBe(true);
  });
});
...
describe("페이지 넘기기", () => {
  it("다음 페이지가 있으면 커서를 넘겨 이어 받는다", async () => {
    ...
    const prRequests = requests.filter((r) =>
      String(r.variables.q).includes("is:pr"),
    );
    expect(prRequests.map((r) => r.variables.after)).toEqual([
      null,
      "cursor-1",
    ]);
  });
