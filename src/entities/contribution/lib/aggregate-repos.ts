...
import { MAX_REPOSITORIES, MAX_REPOSITORIES_SECONDARY, type RepoRef } from "@/shared/api";
...
const SOURCES: {
  ...
  { field: "commits", key: "commitContributionsByRepository", cap: MAX_REPOSITORIES },
  {
    field: "pullRequests",
  ...
  { field: "issues", key: "issueContributionsByRepository", cap: MAX_REPOSITORIES_SECONDARY },
];
...
export function aggregateRepos(
  ...
  const merge = (entries: ContributionsByRepository, field: RepoCountField, cap: number) => {
    ...
    for (const entry of entries) {
      ...
      if (!draft) {
        draft = { repo, counts: { commits: 0, pullRequests: 0, reviews: 0, issues: 0 }, total: 0 };
  ...
  for (const collection of collections) {
    for (const source of SOURCES) merge(collection[source.key], source.field, source.cap);
  }
  ...
  return [...drafts.values()]
    .map(({ repo, counts, total }): RepoStat => {
      ...
      const counted = Object.fromEntries(
        REPO_COUNT_FIELDS.map((field) => [field, known(field) ? counts[field] : null]),
    ...
    .sort((a, b) => b.total - a.total || a.nameWithOwner.localeCompare(b.nameWithOwner));
}
