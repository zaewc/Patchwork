import type { RepoScoring, ScorecardIndex } from "@/entities/repo/model/scoring";
 ...
 */
export async function loadScorecards(scorings: RepoScoring[]): Promise<ScorecardIndex> {
  const keys = [...new Set(scorings.filter((s) => !s.signals.isPrivate).map((s) => s.key))];
