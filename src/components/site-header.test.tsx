...
describe("SiteHeader", () => {
  it("항상 홈으로 가는 브랜드 링크가 있다", () => {
    render(<SiteHeader />);
    expect(screen.getByRole("link", { name: "Patchwork" })).toHaveAttribute("href", "/");
  });
  ...
  it("로그인 후에는 대시보드·README 링크를 보여준다", () => {
    ...
    expect(screen.getByRole("link", { name: "README" })).toHaveAttribute("href", "/export");
  });
  ...
  it("사용자 이름과 avatar로 GitHub 프로필을 잇는다", () => {
    ...
    expect(screen.getByRole("presentation")).toHaveAttribute("src", USER.avatarUrl);
  });
