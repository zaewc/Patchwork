...
describe("mapInBatches", () => {
  it("입력 순서대로 결과를 준다", async () => {
    const doubled = await mapInBatches([1, 2, 3, 4, 5], 2, async (n) => n * 2);
    expect(doubled).toEqual([2, 4, 6, 8, 10]);
  });

  it("빈 목록은 빈 결과다", async () => {
    expect(await mapInBatches([], 4, async (n) => n)).toEqual([]);
  });
  ...
  it("하나가 실패하면 그대로 올린다", async () => {
    await expect(
      mapInBatches([1, 2, 3], 2, async (n) => {
        if (n === 2) throw new Error("두 번째 실패");
        return n;
      }),
