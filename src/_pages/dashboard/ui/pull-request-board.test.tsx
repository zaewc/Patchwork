...
describe("PullRequestBoard", () => {
  ...
  describe("카드", () => {
    it("repository·번호·제목·링크를 담는다", () => {
      render(<PullRequestBoard pullRequests={[pullRequest({ number: 42, title: "fix: 무언가" })]} />);
    ...
    it("마지막 업데이트를 상대 시간으로 보여준다", () => {
      render(<PullRequestBoard pullRequests={[pullRequest({ updatedAt: "2026-08-13T00:00:00Z" })]} />);
    ...
    it("비공개 repository는 Private으로 표시한다", () => {
      render(<PullRequestBoard pullRequests={[pullRequest({ isPrivate: true })]} />);
      expect(screen.getByText("Private")).toBeInTheDocument();
    });

    it("공개 repository에는 Private을 붙이지 않는다", () => {
      render(<PullRequestBoard pullRequests={[pullRequest({ isPrivate: false })]} />);
      expect(screen.queryByText("Private")).not.toBeInTheDocument();
    });

    it("오래 조용한 PR은 Stale로 표시한다", () => {
      render(<PullRequestBoard pullRequests={[pullRequest({ isStale: true })]} />);
