...
describe("ContributionQuilt", () => {
  ...
  it("기여 수를 사분위로 나눠 1~4단계를 준다", () => {
    render(
      <ContributionQuilt weeks={weeksFrom("2026-08-09", [0, 1, 2, 3, 4, 5])} />,
    );
  ...
  it("단계는 기여 수에 따라 단조 증가한다", () => {
    ...
    const levels = counts.map((count, offset) =>
      levelOf(
        cellOf(
          new Date(Date.parse("2026-08-09T00:00:00Z") + offset * DAY_MS)
            .toISOString()
            .slice(0, 10),
          count,
        ),
  ...
  it("주에서 빠진 요일은 빈 칸으로 자리만 채운다", () => {
    ...
    const { container } = render(
      <ContributionQuilt weeks={[[day("2026-08-12", 3)]]} />,
    );
