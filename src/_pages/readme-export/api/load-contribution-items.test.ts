...
const group = (nameWithOwner: string, stars = 50_000) =>
  ({
    ...
    scoring: { key: nameWithOwner, signals: { isPrivate: false, stars, forks: 0 } },
    items: [],
...
describe("loadContributionItems", () => {
  ...
  it("기여한 repository들의 Scorecard를 한 번에 묻는다", async () => {
    ...
    expect(loadScorecards).toHaveBeenCalledExactlyOnceWith([
      { key: "vercel/next.js", signals: { isPrivate: false, stars: 50_000, forks: 0 } },
      { key: "someone/toy", signals: { isPrivate: false, stars: 2, forks: 0 } },
    ]);
  });

  it("Scorecard 총점을 100점으로 환산해 붙인다", async () => {
    vi.mocked(fetchContributionItems).mockResolvedValue([group("vercel/next.js")]);
    vi.mocked(loadScorecards).mockResolvedValue(new Map([["vercel/next.js", 8.5]]));
  ...
  it("deps.dev가 모르는 repository는 외부 관심으로 짐작한다", async () => {
    vi.mocked(fetchContributionItems).mockResolvedValue([group("someone/toy", 2)]);
