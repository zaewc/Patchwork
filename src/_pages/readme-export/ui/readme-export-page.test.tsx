...
import { NOTABLE_IMPACT, PLAIN_IMPACT } from "@/_pages/dashboard/api/dashboard.fixtures";
...
describe("접근 제어", () => {
  it("세션이 없으면 홈으로 보낸다", async () => {
    ...
    await expect(ReadmeExportPage(props())).rejects.toBeInstanceOf(RedirectSignal);
    expect(redirect).toHaveBeenCalledExactlyOnceWith("/");
  });

  it("세션의 토큰과 조회 범위로 기여 목록을 가져온다", async () => {
    await renderPage([group()], { range: "5y" });
    expect(loadContributionItems).toHaveBeenCalledExactlyOnceWith("gho_token", "5y");
  });
...
describe("조회 실패", () => {
  it("사유를 배너로 알리고 다시 시도하라고 안내한다", async () => {
    vi.mocked(loadContributionItems).mockRejectedValue(new Error("GitHub API 오류 (HTTP 403)"));
