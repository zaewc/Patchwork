...
test.describe("지표", () => {
  test("전체 기여 수와 비공개 기여를 보여준다", async ({ page }) => {
    ...
    await expect(card.getByText(String(CONTRIBUTIONS), { exact: true })).toBeVisible();
  ...
  test("주요 OSS 기여와 repository 수를 보여준다", async ({ page }) => {
    ...
    await expect(card.getByText(String(NOTABLE.contributions), { exact: true })).toBeVisible();
  ...
  test("외부 repository 기여 비중을 보여준다", async ({ page }) => {
    ...
    await expect(card.getByText(String(EXTERNAL), { exact: true })).toBeVisible();
  ...
  test("주요 OSS 모드에서는 걸러진 열린 PR 수를 센다", async ({ page }) => {
    ...
    await expect(card.getByText(String(OPEN_PULL_REQUESTS.notable), { exact: true })).toBeVisible();
    await expect(card.getByText(`Stale ${OPEN_PULL_REQUESTS.stale}건`)).toBeVisible();
  });
  ...
  test("전체 모드에서는 GitHub이 준 전체 건수를 쓴다", async ({ page }) => {
    ...
    await expect(
      statCard(page, "Open pull requests").getByText(String(OPEN_PULL_REQUESTS.all), {
        exact: true,
      }),
    ).toBeVisible();
  });
...
test.describe("조회 범위", () => {
  test("기간 탭이 URL과 제목을 함께 바꾼다", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Contributions · 30일" })).toBeVisible();
  });

  test("5년은 구간을 나눠 부르고도 합계를 두 번 세지 않는다", async ({ page }) => {
    await page.getByRole("link", { name: "5년" }).click();

    await expect(page).toHaveURL("/dashboard?range=5y");
    await expect(page.getByRole("heading", { name: "Contributions · 5년" })).toBeVisible();
    await expect(
  ...
  test("URL로 바로 들어가도 그 조건으로 그린다", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "90일" })).toHaveAttribute("aria-current", "page");
    await expect(page.getByRole("link", { name: "전체" })).toHaveAttribute("aria-current", "page");
  });

  test("모르는 범위는 1년으로 떨어진다", async ({ page }) => {
    await page.goto("/dashboard?range=100y");
    await expect(page.getByRole("heading", { name: "Contributions · 1년" })).toBeVisible();
  });
});
...
test.describe("Repositories", () => {
  test("주요 OSS만 남긴다", async ({ page }) => {
    ...
    await expect(table.getByRole("link", { name: "vercel/next.js" })).toBeVisible();
    await expect(table.getByRole("link", { name: "someone/toy-lib" })).toHaveCount(0);
    await expect(table.getByRole("link", { name: "octocat/patchwork" })).toHaveCount(0);
  });
  ...
  test("비공개 repository는 Private으로 표시한다", async ({ page }) => {
    ...
    await expect(section(page, "Repositories").getByText("Private")).toBeVisible();
  });
  ...
  test("항목별 기여 수와 합계를 보여준다", async ({ page }) => {
    ...
    await expect(row.getByRole("cell")).toHaveText(["vercel/next.js", "40", "5", "2", "1", "48"]);
  });
});
...
test.describe("Open pull requests", () => {
  test("검토 상태별로 열을 나눈다", async ({ page }) => {
    ...
    for (const title of ["Changes requested", "Review required", "Approved", "Draft"]) {
      await expect(board.getByRole("heading", { name: new RegExp(`^${title}`) })).toBeVisible();
    }
  });
  ...
  test("PR을 알맞은 열에 놓는다", async ({ page }) => {
    ...
    const column = (title: string) =>
      board.locator("section").filter({ has: page.getByRole("heading", { name: new RegExp(`^${title}`) }) });

    await expect(column("Review required")).toContainText("fix: hydration mismatch");
    await expect(column("Approved")).toContainText("feat: turbopack 플래그 추가");
    await expect(column("Draft")).toContainText("chore: 초안");
    await expect(column("Changes requested")).toContainText("refactor: 변경 요청 받은 PR");
  });
  ...
  test("제목은 GitHub PR로 이어진다", async ({ page }) => {
    ...
    await expect(link).toHaveAttribute("href", "https://github.com/vercel/next.js/pull/101");
  });
...
test.describe("Recently merged", () => {
  test("주요 OSS의 merge된 PR만 남긴다", async ({ page }) => {
    ...
    await expect(merged.getByRole("listitem")).toHaveCount(MERGED_PULL_REQUESTS.notable);
  ...
  test("전체 모드에서는 모두 보여준다", async ({ page }) => {
    ...
    await expect(section(page, "Recently merged").getByRole("listitem")).toHaveCount(
      MERGED_PULL_REQUESTS.all,
    );
  });
});
...
test.describe("일부만 실패했을 때", () => {
  test("PR 조회만 실패하면 경고와 함께 나머지를 보여준다", async ({ page, scenario }) => {
...
test.describe("조회가 실패했을 때", () => {
  test("사유와 재시도 링크를 보여준다", async ({ page, scenario }) => {
    ...
    await expect(
      page.getByText("기여한 Repository가 많아 집계 쿼리가 제한 시간을 넘겼습니다."),
    ).toBeVisible();
  ...
  test("재시도 링크는 조건을 유지한 채 다시 그린다", async ({ page, scenario }) => {
    await scenario("contributions-failure");
    await page.goto("/dashboard?range=90d&scope=all");
    ...
    await expect(page).toHaveURL("/dashboard?range=90d&scope=all");
    await expect(page.getByRole("heading", { name: "Contributions · 90일" })).toBeVisible();
  });

  test("토큰이 만료되면 다시 로그인하도록 안내한다", async ({ page, scenario }) => {
